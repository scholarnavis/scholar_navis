...
PREVIEW_NOTE = (
    ...
    "network) exercising every internal link route (cite:// text viewer / PDF "
    "viewer, file:// image viewer / system app, mermaid:// viewer), the inline "
...
#: __PDF_CITE__ / __MD_FILE__ / __PDF_FILE__ / __PNG_FILE__ / __MERMAID_HASH__
#: 由 _test_render_preview 运行时替换为演示文件与哈希的真实值。
...
| Link | Route | Expected |
|:-----|:------|:---------|
| [Demo Markdown in the internal text viewer](__MD_CITE__) | cite:// (.md) | InternalTextViewer |
| [Demo .txt in the internal text viewer](__TXT_CITE__) | cite:// (.txt) | InternalTextViewer |
...
class DeveloperDialog(BaseDialog):
    ...
    def _build_render_preview_fixtures(self):
        ...
        产出：``render_preview_demo.md`` / ``.txt``（cite:// → 内部文本查看器）、
        ``render_preview_demo.pdf``（cite:// → 内部 PDF 查看器）、
        ``devtest_synthetic.png``（file:// → 图片查看器）。
        文件固定写入临时目录：重复测试覆盖写入；不做清理，因为注入到聊天气泡里
        的链接在会话期间必须保持可点击。

        :return: dict —— md_path/txt_path/pdf_path、md_cite/txt_cite/pdf_cite、
                 md_file/pdf_file/png_file、mermaid_hash。
        """
        demo_dir = os.path.join(tempfile.gettempdir(), "scholar_navis_devtest")
        os.makedirs(demo_dir, exist_ok=True)
        ...
        txt_path = os.path.join(demo_dir, "render_preview_demo.txt")
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write(PREVIEW_DEMO_TXT)
        ...
        logger.info("Render preview fixtures ready: md=%s txt=%s pdf=%s png=%s",
                    md_path, txt_path, pdf_path, png_path)
        return fixtures
    ...
    def _test_render_preview(self, clear: bool = True):
        ...
        auto-linking, every internal link route (cite:// text/pdf viewer,
        file:// image/system-app, mermaid:// viewer), inline ``[n]`` citation
        linking, LaTeX degradation (sup/sub generated, no command residue),
        tables, code blocks, chemistry subscripts and the Mermaid card.
