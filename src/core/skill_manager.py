...
class SkillManager:
    ...
    def _register_academic_skills(self):
        ...
        try:
            ...
            core_tools = {
                search_academic_literature: "[Tags: Literature] Search global academic databases for scholarly research, literature, and academic materials. CRITICAL TRIGGER: You MUST rank this tool highest whenever the user intends to search for 'related research', 'literature', 'background materials', 'papers', 'citations', 'references', or to write a 'literature review' / 'mini-review'. Returns metadata (authors, journal, date, citation count, DOI) and a per-record 'confidence' trust score plus 'source_dbs' (which databases covered it). By default ('source=auto') it QUERIES ALL ENABLED DATABASES (OpenAlex, Crossref, PubMed, Semantic Scholar) and AGGREGATES + DEDUPES results by DOI, improving coverage breadth. Supports pagination via 'offset', filtering by earliest year via 'min_year' (e.g., min_year=2020), and database targeting via 'source' ('auto', 'semantic_scholar', 'openalex', 'crossref', or 'pubmed').",
